...
<header class="site-header w-full bg-white shadow-sm" x-data="{ mobileMenuOpen: false }">
	<nav class="container mx-auto px-4 py-4 flex justify-between items-center" aria-label="Main Navigation">
		...
		<div class="hidden lg:flex flex-grow justify-center px-6">
			<?php get_template_part( 'template-parts/components/header/search-bar' ); ?>
		</div>

		<div class="flex items-center space-x-4">
			<button type="button" class="relative cart-toggle text-gray-800 hover:text-indigo-600 transition" @click="$store.cart.open = true" aria-label="Open cart">
