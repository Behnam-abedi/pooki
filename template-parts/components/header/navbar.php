		...
		<div class="desktop-menu hidden md:flex items-center flex-grow justify-center">
			...
			if ( has_nav_menu( 'primary' ) ) {
				wp_nav_menu( [
					...
					'container'       => false,
					'menu_class'      => 'flex gap-x-10 font-medium text-gray-700',
					'fallback_cb'     => false,
				] );
			} else {
				echo '<ul class="flex gap-x-10 font-medium text-gray-700">';
		...
		<div class="flex items-center gap-x-4 flex-shrink-0">
			...
			<div class="hidden lg:block relative">
				<form role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>" class="relative flex items-center">
					...
					<input type="search" name="s" placeholder="جستجو..." class="w-48 bg-gray-100 text-sm rounded-full py-2.5 pr-10 pl-4 focus:outline-none focus:ring-2 focus:ring-pooki-blue transition-all border-none">
					<button type="submit" class="absolute right-3 text-gray-500 hover:text-pooki-blue">
